...
class KnowledgeBase:
    ...
    def _extract_licenses(
        ...
        # Extrai informações de criticidade
        criticality_pattern = r'(\w+[-\w.]*)\s*\n?\s*(ALTO|MÉDIO|BAIXO)\s*\n?\s*(.+?)(?=\n\n|\Z)'
        criticality_matches = re.findall(criticality_pattern, text, re.IGNORECASE | re.DOTALL)
        
        criticality_map = {}
        for match in criticality_matches:
            license_name = match[0].strip()
            level = match[1].upper()
            reason = match[2].strip()[:200]
            criticality_map[license_name.upper()] = (level, reason)
